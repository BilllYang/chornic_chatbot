...
function find(in){
	...
	var now;
	var quesnumber = 0;
	for(i = 0 i < quesBank[0].content.length - 1; i++){
		quesnumber++
		now = 0;
		match = 0
		for(j = 0 j < in.length - 1; j++){
			if(quesBank[0].content[quesnumber].keyword.includes(in[now]) == true){
				match++
			}
			now++;
		}
		if(match>match_pref){
			match_pref = match
			choose = quesBank[0].content[quesnumber].question
		}
	}
	quesnumber = 0;
	for(i = 0 i < quesBank[1].content.length - 1; i++){
		quesnumber++
		now = 0;
		match = 0
		for(j = 0 j < in.length - 1; j++){
			if(quesBank[1].content[quesnumber].keyword.includes(in[now]) == true){
				match++
			}
			now++
		}
		if(match>match_pref){
			match_pref = match
			choose = quesBank[1].content[quesnumber].question
		}
	}
	quesnumber = 0;
	for(i = 0 i < quesBank[2].content.length - 1; i++){
		quesnumber++
		now = 0;
		match = 0
		for(j = 0 j < in.length - 1; j++){
			if(quesBank[2].content[quesnumber].keyword.includes(in[now]) == true){
				match++
			}
			now++
		}
		if(match>match_pref){
			match_pref = match
			choose = quesBank[2].content[quesnumber].question
		}
	}
	quesnumber = 0;
	for(i = 0 i < quesBank[3].content.length - 1; i++){
		quesnumber++
		now = 0;
		match = 0
		for(j = 0 j < in.length - 1; j++){
			if(quesBank[3].content[quesnumber].keyword.includes(in[now]) == true){
				match++
			}
			now++
		}
		if(match>match_pref){
			match_pref = match
			choose = quesBank[3].content[quesnumber].question
		}
	}
}
